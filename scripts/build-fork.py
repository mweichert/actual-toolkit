...
This script lives OUTSIDE the git repository, in a parent directory.
Repository: ../actual/
Config: ../fork.yaml
...
class ForkBuilder:
    ...
    def _get_next_fork_tag(self, version: str) -> str:
        ...
        result = subprocess.run(
            ["git", "-C", str(self.repo_root), "tag", "-l", f"{version}-fork.*"],
            capture_output=True,
            text=True,
            check=True,
        )
    ...
    def run(self) -> None:
        ...
        result = subprocess.run(
            ["git", "-C", str(self.repo_root), "diff-index", "--quiet", "HEAD", "--"],
            capture_output=True,
        )
        if result.returncode != 0:
        ...
        # Show commits in fork (only if fork branch exists and not in dry-run)
        if not self.dry_run and self._get_commit("fork"):
            print(f"\nCommits in fork ahead of {self.config.base}:")
            subprocess.run(["git", "-C", str(self.repo_root), "log", "--oneline", f"{self.config.base}..fork"])
...
What this script does:
  1. Sync      Fetch upstream, reset 'master' to upstream/master, push master
  2. Rebase    Rebase each branch onto its base (topologically sorted)
  3. Build     Create 'fork' from master, merge all branches, push fork
  4. Tag       If fork changed, create and push tag (e.g., 0.0.1-fork.1)

All changes are pushed to origin automatically.

Configuration is read from fork.yaml in the parent directory.
Repository is expected at ../actual/ relative to this script.
        """,
    ...
    # Find paths - script is in scripts/, config and repo are siblings in parent
    script_dir = Path(__file__).parent
    workspace_root = script_dir.parent
    config_path = workspace_root / "fork.yaml"
    repo_root = workspace_root / "actual"
